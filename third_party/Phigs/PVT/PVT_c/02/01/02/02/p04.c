/* fort/02/01/02/02/p04.f -- translated by f2c (version of 22 July 1992  22:54:52).
...
/* open workstation */
    ...
    setmsg_("1 3 4", "When the reference handling flag is set to delete and "
	    "the open specified structure is referenced outside the structure"
	    " network the following changes should occur to the CSS: All clos"
	    "ed structures contained in a structure network are removed. The "
	    "specified structure is OPEN and EMPTY.", 5L, 284L);
#else /* NO_PROTO */
    setmsg_("1 3 4", "When the reference handling flag is set to delete and \
the open specified structure is referenced outside the structure network the\
 following changes should occur to the CSS: All closed structures contained \
in a structure network are removed. The specified structure is OPEN and EMPT\
Y.", 5L, 284L);
#endif /* NO_PROTO */
/* specf = open specified structure (and posted specified structure) */
    ...
    L__1 = csseq_("100, 1, 101, 1, 103, 0/100, 1, 101, 2, 103, 0/100, 2, 102"
	    ...
	    "2, 105, 1, 110, 0/106, 0/120, 0/130, 1, 107, 1, 113, 0/140, 1, 1"
	    "42, 0/", 191L);
#else /* NO_PROTO */
    L__1 = csseq_("100, 1, 101, 1, 103, 0/100, 1, 101, 2, 103, 0/100, 2, 102\
, 1, 104, 1, 109, 0/100, 2, 102, 1, 104, 2, 109, 0/100, 2, 102, 2, 105, 1, 1\
10, 0/106, 0/120, 0/130, 1, 107, 1, 113, 0/140, 1, 142, 0/", 191L);
#endif /* NO_PROTO */
    ifpf_(&L__1);
#ifndef NO_PROTO
    setmsg_("1", "<Delete structure network> with the reference flag set to "
	    "delete and an open specified structure which is referenced outsi"
	    "de the structure network should cause that structure to exist in"
	    " the CSS as an open structure.", 1L, 216L);
#else /* NO_PROTO */
    setmsg_("1", "<Delete structure network> with the reference flag set to \
delete and an open specified structure which is referenced outside the struc\
ture network should cause that structure to exist in the CSS as an open stru\
cture.", 1L, 216L);
#endif /* NO_PROTO */
...
/* strref = structure id */
    ...
    setmsg_("1", "<Delete structure network> with the reference flag set to "
	    "delete and an open specified structure which is referenced outsi"
	    "de the structure network should cause that structure to exist in"
	    " the CSS as an empty structure.", 1L, 217L);
#else /* NO_PROTO */
    setmsg_("1", "<Delete structure network> with the reference flag set to \
delete and an open specified structure which is referenced outside the struc\
ture network should cause that structure to exist in the CSS as an empty str\
ucture.", 1L, 217L);
#endif /* NO_PROTO */
...
/* strsti = structure status identifier */
    ...
    setmsg_("10", "<Delete structure network> should cause the open specifie"
	    ...
	    " set to delete.", 2L, 200L);
#else /* NO_PROTO */
    setmsg_("10", "<Delete structure network> should cause the open specifie\
d structure referenced outside the structure network to become unposted from\
 the workstation when the reference handling flag is set to delete.", 2L, 
	    200L);
#endif /* NO_PROTO */
