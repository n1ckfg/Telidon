/* fort/02/01/02/02/p07.f -- translated by f2c (version of 22 July 1992  22:54:52).
...
/* open workstation */
    ...
    setmsg_("1 3 4", "When the reference handling flag is set to delete and "
	    "the open non-specified structure is not referenced outside the s"
	    "tructure network the following changes should occur to the CSS: "
	    "All closed structures contained in a structure network are remov"
	    "ed. The open non-specified structure is OPEN and EMPTY.", 5L, 
	    301L);
#else /* NO_PROTO */
    setmsg_("1 3 4", "When the reference handling flag is set to delete and \
the open non-specified structure is not referenced outside the structure net\
work the following changes should occur to the CSS: All closed structures co\
ntained in a structure network are removed. The open non-specified structure\
 is OPEN and EMPTY.", 5L, 301L);
#endif /* NO_PROTO */
...
/* nospec = open non-specified structure */
    ...
    L__1 = csseq_("100, 1, 101, 1, 103, 0/100, 1, 101, 2, 103, 0/100, 2, 102"
	    ...
	    "2, 105, 1, 110, 0/111, 0/130, 1, 107, 1, 113, 0/140, 1, 142, 0/", 
	    184L);
#else /* NO_PROTO */
    L__1 = csseq_("100, 1, 101, 1, 103, 0/100, 1, 101, 2, 103, 0/100, 2, 102\
, 1, 104, 1, 109, 0/100, 2, 102, 1, 104, 2, 109, 0/100, 2, 102, 2, 105, 1, 1\
10, 0/111, 0/130, 1, 107, 1, 113, 0/140, 1, 142, 0/", 184L);
#endif /* NO_PROTO */
    ifpf_(&L__1);
#ifndef NO_PROTO
    setmsg_("1", "<Delete structure network> with the reference flag set to "
	    ...
	    "k and not referenced outside the structure network should cause "
	    "that structure to exist in the CSS as an open structure.", 1L, 
	    242L);
#else /* NO_PROTO */
    setmsg_("1", "<Delete structure network> with the reference flag set to \
delete and an open non-specified structure present in the network and not re\
ferenced outside the structure network should cause that structure to exist \
in the CSS as an open structure.", 1L, 242L);
#endif /* NO_PROTO */
...
/* strref = structure id */
    ...
    setmsg_("1", "<Delete structure network> with the reference flag set to "
	    "delete and an open non-specified structure present in the networ"
	    "k and not referenced outside the structure network should cause "
	    "that structure to exist in the CSS as an empty structure.", 1L, 
	    243L);
#else /* NO_PROTO */
    setmsg_("1", "<Delete structure network> with the reference flag set to \
delete and an open non-specified structure present in the network and not re\
ferenced outside the structure network should cause that structure to exist \
in the CSS as an empty structure.", 1L, 243L);
#endif /* NO_PROTO */
/* use <inquire structure status> on non-specified structure to determine 
*/
/* strsti = structure status identifier */
    ...
    setmsg_("10", "<Delete structure network> should cause the closed posted"
	    " specified and an open posted non-specified structure not refere"
	    "nced outside the structure network to become unposted from a wor"
	    "kstation when the reference handling flag is set to delete.", 2L, 
	    244L);
#else /* NO_PROTO */
    setmsg_("10", "<Delete structure network> should cause the closed posted\
 specified and an open posted non-specified structure not referenced outside\
 the structure network to become unposted from a workstation when the refere\
nce handling flag is set to delete.", 2L, 244L);
#endif /* NO_PROTO */
