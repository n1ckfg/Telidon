/* fort/02/01/02/02/p02.f -- translated by f2c (version of 22 July 1992  22:54:52).
...
/* PHIGS parameter types */
    initgl_("02.01.02.02/02", 14L);
/* open PHIGS */
    xpopph_(&globnu_1.errfil, &globnu_1.memun);
    delcss_();
#ifndef NO_PROTO
    setmsg_("3 4", "When the reference handling flag is set to delete and th"
	    "e closed specified structure is referenced outside the structure"
	    " network the following changes should occur to the CSS: All clos"
	    "ed structures contained in a structure network are removed.", 3L, 
	    243L);
#else /* NO_PROTO */
    setmsg_("3 4", "When the reference handling flag is set to delete and th\
e closed specified structure is referenced outside the structure network the\
 following changes should occur to the CSS: All closed structures contained \
in a structure network are removed.", 3L, 243L);
#endif /* NO_PROTO */
