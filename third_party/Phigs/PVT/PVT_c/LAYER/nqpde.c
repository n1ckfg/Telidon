...
/**************************************************************************
NAME
     INQUIRE PATHS TO DESCENDANTS - obtain descendants of  speci-
     fied structure

  FORTRAN Syntax
     SUBROUTINE pqpde ( STRID, PTHORD, PTHDEP, IPTHSZ, N, ERRIND, OL,
                              APTHSZ, PATHS )
...
**************************************************************************/
#ifdef NO_PROTO
nqpde (strid, pthord, pthdep, ipthsz, n, errind, ol, apthsz, fpath)
   integer      *strid,
            *pthord,
            *pthdep,
            *ipthsz,
            ...
            *apthsz,
             fpath[][2];
#else
nqpde (integer *strid, integer *pthord, integer *pthdep, integer *ipthsz, 
       integer *n, integer *errind,
...
/**************************************************************************
  C Syntax
     void
     pinq_paths_descs ( struct_id, order, depth, store, errind, paths )
...
**************************************************************************/
   ...
   pinq_paths_descs ((Pint) *strid, order, (Pint) *pthdep, store, 
                     &cerror, &path);
   *errind = cerror;
