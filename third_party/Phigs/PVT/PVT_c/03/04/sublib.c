/* fort/03/04/sublib.f -- translated by f2c (version of 22 July 1992  22:54:52).
...
/*  ********************************************************* */
/*  *                                                       * */
/*  *    SUBROUTINE 03.04/arcpth                            * */
/*  *                                                       * */
/*  *    PHIGS Validation Tests, produced by NIST           * */
/*  *                                                       * */
/*  ********************************************************* */
#ifndef NO_PROTO
/* Subroutine */ int arcpth_(integer *arid, char *adsw, integer *strid, 
	integer *pthord, integer *pthdep, integer *retcod, char *expctd, 
	ftnlen adsw_len, ftnlen expctd_len)
#else /* NO_PROTO */
/* Subroutine */ int arcpth_(arid, adsw, strid, pthord, pthdep, retcod, 
	expctd, adsw_len, expctd_len)
integer *arid;
char *adsw;
...
#endif /* NO_PROTO */
{
    ...
    extern /* Subroutine */ int inmsg_(char *, ftnlen), unmsg_(char *, ftnlen)
	    ;
#else /* NO_PROTO */
    extern /* Subroutine */ int inmsg_(), unmsg_();
#endif /* NO_PROTO */
    static logical ancest;
#ifndef NO_PROTO
    extern /* Subroutine */ int nrepde (integer *, integer *, integer *, 
	    integer *, integer *, integer *, integer *, integer *, integer *);

#else /* NO_PROTO */
    extern /* Subroutine */ int nrepde ();
#endif /* NO_PROTO */
    static integer replen;
#ifndef NO_PROTO
    extern /* Subroutine */ int nrepan (integer *, integer *, integer *, 
	    integer *, integer *, integer *, integer *, integer *, integer *);

#else /* NO_PROTO */
    extern /* Subroutine */ int nrepan ();
#endif /* NO_PROTO */
    static integer explen[20], reppth[50], repnum, exppth[1000]	/* was [50][
	    20] */, expnum;

    /* Fortran I/O blocks */
    static icilist io___9 = { 0, msg, 0, "(A,I4,A,I4,A,A,A, 3I5)", 300, 1 };
    static icilist io___13 = { 0, msg, 0, "(A,A,A, 3I5)", 300, 1 };


/*  Arcpth obtains a set of ancestor or descendant paths from the */
/*  specified archive file and compares the reported set to an */
/*  expected value to test for set-equality, where each element of */
/*  the set is a path. The result is returned in retcod, which is */
/*  zero if the sets are equal.  If the sets are not equal, arcpth */
/*  writes out an informative message explaining the conditions of */
/*  failure. */
/*  --- Input Parameters: */
...
/* Writing concatenation */
	i__1[0] = 23, a__1[0] = "Invalid adsw in ARCPTH:";
...
/* first get number of paths = repnum */
    if (ancest) {
	nrepan (arid, strid, pthord, pthdep, &c__25, &c__0, &repnum, &replen, 
		reppth);
    } else {
	nrepde (arid, strid, pthord, pthdep, &c__25, &c__0, &repnum, &replen, 
		reppth);
    }
/* if number of paths unequal, whole set is unequal */
	...
	*retcod = 2;
	...
	do_fio(&c__1, "), ", 3L);
	do_fio(&c__1, "resulting from retrieve with structure-id, path ", 48L)
		;
...
/* get next path */
	if (ancest) {
	    nrepan (arid, strid, pthord, pthdep, &c__25, &ipath, &idum, &
		    replen, reppth);
	} else {
	    nrepde (arid, strid, pthord, pthdep, &c__25, &ipath, &idum, &
		    replen, reppth);
	}
