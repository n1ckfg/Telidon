/* fort/04/02/03/02/p12.f -- translated by f2c (version of 22 July 1992  22:54:52).
...
static real c_b58 = .1f;
#else /* NO_PROTO */
static real c_b57 = (float)1.;
static real c_b58 = (float).1;
#endif /* NO_PROTO */
static integer c__33 = 33;
#ifndef NO_PROTO
static real c_b64 = .4f;
static real c_b66 = .5f;
#else /* NO_PROTO */
static real c_b64 = (float).4;
static real c_b66 = (float).5;
#endif /* NO_PROTO */
...
#ifndef NO_PROTO
static real c_b138 = .25f;
static real c_b140 = .3f;
static real c_b151 = 0.f;
static real c_b171 = 3.f;
#else /* NO_PROTO */
static real c_b138 = (float).25;
static real c_b140 = (float).3;
static real c_b151 = (float)0.;
static real c_b171 = (float)3.;
#endif /* NO_PROTO */

/*  ********************************************************* */
/*  *                                                       * */
/*  *    TEST NUMBER: 04.02.03.02/12                        * */
...
#endif /* NO_PROTO */
{
    ...
    extern /* Subroutine */ int nspli (integer *), nsplr (integer *, integer *
	    , integer *, real *, integer *), nopst (integer *), nstxi (
	    integer *), nsvwi (integer *), nstxr (integer *, integer *, 
	    ...
	    ftnlen);
#else /* NO_PROTO */
    extern /* Subroutine */ int nspli (), nsplr (), nopst (), nstxi (), 
	    nsvwi (), nstxr (), nqtxx ();
#endif /* NO_PROTO */
    static real offact;
#ifndef NO_PROTO
    ...
    extern /* Subroutine */ int initgl_(char *, ftnlen), xpopph_(integer *, 
	    ...
	    rnperm_(integer *, integer *), numlab_(integer *, real *, real *, 
	    real *), srtiar_(integer *, integer *), dilist_(char *, integer *,
    ...
    extern /* Subroutine */ int initgl_(), xpopph_(), setasf_(), setmsg_(), 
	    nstxal (), nseplb (), rnperm_(), numlab_(), srtiar_(), dilist_(), 
	    opfail_(), tstasc_(), chkinq_(), dchpfv_();
#endif /* NO_PROTO */
...
#ifndef NO_PROTO
    setmsg_("3 5 6 34 38 39 40 41", "Fonts 1 and 2 should both contain all t"
...
/*        skip unavailable font-precision pairs */
	    ...
	    nstxr (&globnu_1.wkid, &c__3, &font, &prec, &c_b57, &c_b58, &c__1)
		    ;
	    nstxi (&c__3);
	    nseplb (&c__2);
...
/* Computing MIN */
		...
		numlab_(&numlft, &c_b64, &ypos, &yincr);
		i__1 = numlft;
		for (icx = 1; icx <= i__1; ++icx) {
		    iy = icx + grpbas;
		    ntx (&c_b66, &ypos, ascch + (perm[iy - 1] - 1), 1L);
...
/* Writing concatenation */
    ...
    tstasc_("ASCII DIGITS: Which character is NOT a digit?", ch__2, "U", 45L, 
	    11L, 1L);
/*  clear out everything */
    nsep (&c__1);
    ndellb (&c__1, &c__3);
#ifndef NO_PROTO
    setmsg_("3 5 6 34 38 39 40 42", "Text fonts 1 and 2 should be visually d"
...
/*  switch fonts */
	fonta = 7 - fonta;
	fontb = 7 - fontb;
	nstxr (&globnu_1.wkid, &c__3, &c__1, &prec, &c_b57, &c_b58, &c__1);
	nstxr (&globnu_1.wkid, &c__4, &c__2, &prec, &c_b57, &c_b58, &c__1);
	ypos = 1 - yincr;
	numlab_(&numlin, &c_b138, &ypos, &yincr);
	i__1 = numlin;
	for (ix = 1; ix <= i__1; ++ix) {
	    if (ix == txdif) {
		nstxi (&fontb);
...
L690:
#ifndef NO_PROTO
    setmsg_("3 5 6 38 39 43 44 45 50 57 58 61 62", "In text font 1, all char"
	    "acter bodies affected by the same character height, character ex"
	    "pansion factor, and character spacing should have the same width."
	    , 35L, 153L);
#else /* NO_PROTO */
    setmsg_("3 5 6 38 39 43 44 45 50 57 58 61 62", "In text font 1, all char\
acter bodies affected by the same character height, character expansion fact\
or, and character spacing should have the same width.", 35L, 153L);
#endif /* NO_PROTO */
/*  Use <inquire text extent> to determine */
/*    ncwh = nominal character width/height ratio for font 1 */
    nqtxx (&dialog_1.specwt, &c__1, &c_b57, &c_b151, &c_b57, &c__0, &c__1, &
...
/*  try to paint text and polylines different colors */
    nspli (&c__1);
    nsplr (&globnu_1.wkid, &c__1, &c__1, &c_b57, &c__2);
/*  narrow characters */
