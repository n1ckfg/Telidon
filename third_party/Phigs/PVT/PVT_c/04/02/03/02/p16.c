/* fort/04/02/03/02/p16.f -- translated by f2c (version of 22 July 1992  22:54:52).
...
#ifndef NO_PROTO
static real c_b49 = 3.f;
#else /* NO_PROTO */
static real c_b49 = (float)3.;
#endif /* NO_PROTO */
static integer c__3 = 3;
#ifndef NO_PROTO
static real c_b75 = .1f;
#else /* NO_PROTO */
...
#endif /* NO_PROTO */
{
    ...
    extern /* Subroutine */ int nsplr (integer *, integer *, integer *, real *
	    , integer *), nexst (integer *), nopst (integer *), nstxi (
	    integer *), nsvwi (integer *), nstxr (integer *, integer *, 
	    integer *, integer *, real *, real *, integer *);
#else /* NO_PROTO */
    extern /* Subroutine */ int nsplr (), nexst (), nopst (), nstxi (), 
	    nsvwi (), nstxr ();
#endif /* NO_PROTO */
    ...
    extern /* Subroutine */ int initgl_(), xpopph_(), setasf_(), setmsg_(), 
	    nqwksl (), rnbset_(), rnperm_(), nsiasf (), nstxci (), numlab_();
#endif /* NO_PROTO */
...
/*  use bundle attributes */
    ...
    nopst (&c__102);
#ifndef NO_PROTO
    setmsg_("3 5 6 64 65", "A defined text color index should cause the addr"
...
/*  set text representation ix : color = txtcl */
	nstxr (&globnu_1.wkid, &ix, &c__1, &prec, &c_b29, &c_b30, &txtci);
/*  draw text line #ix with text colour index = txcol(ix) */
	ntx (&c_b30, &yloc, "Defined text color", 18L);
	yloc -= yincr;
/* L2100: */
    }
...
/*  clear out last display from structure */
    nemst (&c__102);
#ifndef NO_PROTO
    setmsg_("3 5 6 63 64 66", "An undefined text color index should cause en"
	    "try number 1 in the color table to be used when rendering a text"
	    " primitive.", 14L, 120L);
#else /* NO_PROTO */
    setmsg_("3 5 6 63 64 66", "An undefined text color index should cause en\
try number 1 in the color table to be used when rendering a text primitive.", 
	    14L, 120L);
#endif /* NO_PROTO */
...
/* L2400: */
    ...
    nspli (&c__1);
    nsplr (&globnu_1.wkid, &c__1, &c__1, &c_b49, &c__1);
    npl (&c__6, xa, ya);
#ifndef NO_PROTO
    yloc = .5f;
    yincr = .0625f;
...
/*  set text representations #1-4 */
    nstxr (&globnu_1.wkid, &c__1, &c__1, &c__0, &c_b29, &c_b30, undf);
    nstxr (&globnu_1.wkid, &c__2, &c__1, &c__2, &c_b29, &c_b30, &undf[1]);
    nstxr (&globnu_1.wkid, &c__3, &c__1, &c__1, &c_b29, &c_b30, &undf[2]);
    nstxr (&globnu_1.wkid, &c__4, &c__1, &c__2, &c_b29, &c_b30, &c__1);
/*  display interleaved: */
...
/*    explct text lines of color #1 */
	...
	ntx (&c_b75, &yloc, "Undefined color", 15L);
	yloc -= yincr;
/* L2300: */
    }
#ifndef NO_PROTO
