/* fort/04/02/04/01/p03.f -- translated by f2c (version of 22 July 1992  22:54:52).
...
#ifndef NO_PROTO
static real c_b46 = .04f;
static real c_b55 = .95f;
#else /* NO_PROTO */
static real c_b46 = (float).04;
...
#else /* NO_PROTO */
static real c_b63 = (float)0.;
static real c_b67 = (float)1.;
#endif /* NO_PROTO */
static integer c__5 = 5;
#ifndef NO_PROTO
static real c_b76 = .3f;
static real c_b77 = 3.f;
#else /* NO_PROTO */
static real c_b76 = (float).3;
static real c_b77 = (float)3.;
#endif /* NO_PROTO */
static integer c__4 = 4;
static integer c__26 = 26;
#ifndef NO_PROTO
static real c_b128 = .25f;
#else /* NO_PROTO */
static real c_b128 = (float).25;
#endif /* NO_PROTO */

/*  ********************************************************* */
/*  *                                                       * */
/*  *    TEST NUMBER: 04.02.04.01/03                        * */
...
#endif /* NO_PROTO */
{
...
#endif /* NO_PROTO */
    static integer txci;
    static real npcx, npcy, rbox;
#ifndef NO_PROTO
    extern /* Subroutine */ int pass_(void), nsln (integer *);
#else /* NO_PROTO */
    extern /* Subroutine */ int pass_(), nsln ();
#endif /* NO_PROTO */
    static integer lunx;
...
#endif /* NO_PROTO */
    static real chsiz, xleft, xincr, yincr;
#ifndef NO_PROTO
    ...
    extern /* Subroutine */ int nclst (void), dynpf_(char *, char *, ftnlen, 
	    ftnlen), nemst (integer *);
#else /* NO_PROTO */
    extern /* Subroutine */ int nclst (), dynpf_(), nemst ();
#endif /* NO_PROTO */
    static integer lstrk[200];
#ifndef NO_PROTO
    extern /* Subroutine */ int nsatp (integer *), wcnpc_(real *, real *, 
	    real *, real *, real *);
#else /* NO_PROTO */
    extern /* Subroutine */ int nsatp (), wcnpc_();
#endif /* NO_PROTO */
    ...
    extern /* Subroutine */ int nopst (integer *), nqtxf (integer *, integer *
	    ...
	    rnset_(integer *, integer *, integer *), unrep_(integer *, 
	    integer *, integer *, char *, char *, ftnlen, ftnlen), nqtxx (
	    integer *, integer *, real *, real *, real *, integer *, integer *
	    , integer *, char *, integer *, real *, real *, real *, real *, 
	    ftnlen);
#else /* NO_PROTO */
    extern /* Subroutine */ int nopst (), nqtxf (), nsvwi (), rnset_(), 
	    unrep_(), nqtxx ();
#endif /* NO_PROTO */
    ...
    extern /* Subroutine */ int initgl_(char *, ftnlen), xpopph_(integer *, 
	    ...
	    integer *, char *, logical *, char *, ftnlen, ftnlen), nstxpr (
	    integer *), nstxal (integer *, integer *), nschsp (real *), 
	    nstxci (integer *), nsplci (integer *), nsatal (integer *, 
	    integer *), nstxfn (integer *), nsatch (real *), nschxp (real *), 
	    dchpfv_(char *, integer *, integer *, ftnlen), dilist_(char *, 
	    integer *, integer *, ftnlen), numlab_(integer *, real *, real *, 
	    real *), rnperm_(integer *, integer *);
#else /* NO_PROTO */
    extern /* Subroutine */ int initgl_(), xpopph_(), setdlg_(), setasf_(), 
	    chkinq_(), setmsg_(), chkfnt_(), nstxpr (), nstxal (), nschsp (), 
	    nstxci (), nsplci (), nsatal (), nstxfn (), nsatch (), nschxp (), 
	    dchpfv_(), dilist_(), numlab_(), rnperm_();
#endif /* NO_PROTO */

    /* Fortran I/O blocks */
    static icilist io___36 = { 0, vlbl, 0, "(I1,A)", 2, 1 };
...
/* L100: */
    ...
    setmsg_("5 6 12 14", "All positive text fonts should appear as specified"
	    " in the ISO register.", 9L, 71L);
    inmsg_("SR 12 is indirectly confirmed in the following test case since t"
...
#ifndef NO_PROTO
    chkfnt_(&lposx, lposfn, lpospr, &maxfnt, chstr, (logical*)&c__0, "A", 10L,
	     1L);
#else /* NO_PROTO */
    chkfnt_(&lposx, lposfn, lpospr, &maxfnt, chstr, &c__0, "A", 10L, 1L);
#endif /* NO_PROTO */
    dynpf_(msg, "y", 300L, 1L);
    nemst (&c__102);
#ifndef NO_PROTO
    setmsg_("5 6 12 14 21", "All positive text fonts should appear as specif"
	    "ied in the ISO register, even when less precision is requested t"
	    "han is available for the font.", 12L, 141L);
#else /* NO_PROTO */
    setmsg_("5 6 12 14 21", "All positive text fonts should appear as specif\
ied in the ISO register, even when less precision is requested than is avail\
able for the font.", 12L, 141L);
#endif /* NO_PROTO */
/*  call routine to handle display */
#ifndef NO_PROTO
    chkfnt_(&lposx, lposfn, lpospr, &maxfnt, chstr, (logical*)&c__1, "A", 10L,
	     1L);
#else /* NO_PROTO */
    chkfnt_(&lposx, lposfn, lpospr, &maxfnt, chstr, &c__1, "A", 10L, 1L);
#endif /* NO_PROTO */
    dynpf_(msg, "y", 300L, 1L);
    nemst (&c__102);
/*  end_pos: */
L190:
    ...
    setmsg_("5 6 12 14", "All non-positive text fonts should be workstation "
	    "independent and appear as specified in the implementor's documen"
	    "tation.", 9L, 121L);
#else /* NO_PROTO */
    s_copy(msg, "IMPLEMENTOR DEFINED TEXT FONTS: Answer y if both are true, \
...
he same font on all workstation types which support that identifier.", 300L, 
	    279L);
    setmsg_("5 6 12 14", "All non-positive text fonts should be workstation \
independent and appear as specified in the implementor's documentation.", 9L, 
	    121L);
#endif /* NO_PROTO */
/*  call routine to handle display */
#ifndef NO_PROTO
    chkfnt_(&lnegx, lnegfn, lnegpr, &maxfnt, chstr, (logical*)&c__0, "A", 10L,
	     1L);
#else /* NO_PROTO */
    chkfnt_(&lnegx, lnegfn, lnegpr, &maxfnt, chstr, &c__0, "A", 10L, 1L);
#endif /* NO_PROTO */
    dynpf_(msg, "y", 300L, 1L);
    nemst (&c__102);
#ifndef NO_PROTO
    setmsg_("5 6 12 14 21", "All non-positive text fonts should be workstati"
	    "on independent and appear as specified in the implementor's docu"
	    "mentation, even when less precision is requested than is availab"
	    "le for the font.", 12L, 191L);
#else /* NO_PROTO */
    setmsg_("5 6 12 14 21", "All non-positive text fonts should be workstati\
on independent and appear as specified in the implementor's documentation, e\
ven when less precision is requested than is available for the font.", 12L, 
	    191L);
#endif /* NO_PROTO */
/*  call routine to handle display */
#ifndef NO_PROTO
    chkfnt_(&lnegx, lnegfn, lnegpr, &maxfnt, chstr, (logical*)&c__1, "A", 10L,
	     1L);
#else /* NO_PROTO */
    chkfnt_(&lnegx, lnegfn, lnegpr, &maxfnt, chstr, &c__1, "A", 10L, 1L);
#endif /* NO_PROTO */
    dynpf_(msg, "y", 300L, 1L);
    nemst (&c__102);
/*  end_font_id: */
L290:
#ifndef NO_PROTO
    setmsg_("5 6 9 10 23 27 33 49", "In each text font, all character bodies"
	    " affected by the same annotation text character height, characte"
	    "r expansion factor, and character spacing should have the same h"
	    "eight.", 20L, 173L);
#else /* NO_PROTO */
    setmsg_("5 6 9 10 23 27 33 49", "In each text font, all character bodies\
 affected by the same annotation text character height, character expansion \
factor, and character spacing should have the same height.", 20L, 173L);
#endif /* NO_PROTO */
    nstxpr (&c__2);
    nschh (&c_b46);
    nstxal (&c__1, &c__1);
...
/*  try to paint text and polylines different colors */
    ...
    nsplci (&c__2);
    nsatal (&c__1, &c__1);
    wcnpc_(&c_b63, &c_b63, &npcx, &npcy, &npcpwc);
    i__1 = numcol;
    for (ix = 1; ix <= i__1; ++ix) {
...
/*          requested character height)  for txfnt */
	nqtxx (&dialog_1.specwt, &txfnt, &c_b67, &c_b63, &c_b67, &c__3, &c__1,
...
/*  display distrg at location ix */
	natr (&xpos, &ytop, &c_b63, &c_b63, distrg, numch);
...
aracters NOT aligned between the separators?", &numcol, &ngcol, 107L);
    ...
    nemst (&c__102);
#ifndef NO_PROTO
    setmsg_("1 2 5 7 9 10 23 48 49 66", "Annotation text character height ma"
	    "gnitude should specify the nominal height, in TLC, from baseline"
	    " to capline, of all capital letters within a font.", 24L, 149L);
    dilist_("ANNOTATION TEXT CHARACTER HEIGHT FOR SIZE OF CAPITAL LETTERS: E"
	    "nter list of font identifiers available in stroke precision, oth"
	    "er than 1 and 2, which support alphabetic characters (n if none)."
	    , &alfsiz, alfont, 192L);
#else /* NO_PROTO */
    setmsg_("1 2 5 7 9 10 23 48 49 66", "Annotation text character height ma\
gnitude should specify the nominal height, in TLC, from baseline to capline,\
 of all capital letters within a font.", 24L, 149L);
    dilist_("ANNOTATION TEXT CHARACTER HEIGHT FOR SIZE OF CAPITAL LETTERS: E\
nter list of font identifiers available in stroke precision, other than 1 an\
d 2, which support alphabetic characters (n if none).", &alfsiz, alfont, 192L)
	    ;
#endif /* NO_PROTO */
/*  add 1 and 2 */
    alfont[alfsiz] = 1;
...
/*  display and label str, using chht and curfnt */
	...
	r__2 = lbox + (float).1;
#endif /* NO_PROTO */
	natr (&r__2, &ypos, &c_b63, &c_b63, str, 3L);
	ypos -= yincr;
/*  next ix */
/* L700: */
    }
#ifndef NO_PROTO
    dchpfv_("ANNOTATION TEXT CHARACTER HEIGHT FOR SIZE OF CAPITAL LETTERS: I"
	    "n which line do the upper-case letters NOT fit exactly between t"
	    "he dashed baseline and the dotted capline?", &numlin, &ngline, 
	    169L);
#else /* NO_PROTO */
    dchpfv_("ANNOTATION TEXT CHARACTER HEIGHT FOR SIZE OF CAPITAL LETTERS: I\
n which line do the upper-case letters NOT fit exactly between the dashed ba\
seline and the dotted capline?", &numlin, &ngline, 169L);
#endif /* NO_PROTO */
    nemst (&c__102);
#ifndef NO_PROTO
...
#ifndef NO_PROTO
	inmsg_("Cannot test SR14, because there are no unrepresentable chara"
...
#ifndef NO_PROTO
    setmsg_("6 7 13", "There should be a implementation dependent way to dep"
	    "ict a character code for which a font has no graphic representat"
	    "ion.", 6L, 121L);
#else /* NO_PROTO */
    setmsg_("6 7 13", "There should be a implementation dependent way to dep\
ict a character code for which a font has no graphic representation.", 6L, 
	    121L);
#endif /* NO_PROTO */
...
/*  check font 1 - if no good, fail and skip out */
    unrep_(&c__1, &unr1sz, unr1ls, "A", result, 1L, 1L);
    if (*result == 'F') {
	fail_();
	goto L890;
    }
...
/*  do_f2: */
L850:
    unrep_(&c__2, &unr2sz, unr2ls, "A", result, 1L, 1L);
    L__1 = *result == 'P';
    ifpf_(&L__1);
/*  end_unrep: */
L890:
#ifndef NO_PROTO
    setmsg_("5 6 8 15 19", "If annotation text relative primitive requests a"
	    "n unavailable font or precision, font=1 and precision=STRING sho"
	    "uld be used.", 11L, 124L);
#else /* NO_PROTO */
    setmsg_("5 6 8 15 19", "If annotation text relative primitive requests a\
n unavailable font or precision, font=1 and precision=STRING should be used.",
	     11L, 124L);
#endif /* NO_PROTO */
...
/*  label all lines from 1 to numlin */
    ...
    ypos = 1 - yincr;
/*  draw labels */
    numlab_(&numlin, &c_b128, &ypos, &yincr);
    nsatal (&c__1, &c__3);
/*  allow room within picture square = (x-space - 0.1) / #characters */
/* Computing MIN */
    r__1 = yincr / 2;
#ifndef NO_PROTO
    r__2 = dmin(r__1,.045454545454545456f) * npcpwc;
#else /* NO_PROTO */
    r__2 = dmin(r__1,(float).045454545454545456) * npcpwc;
#endif /* NO_PROTO */
    nsatch (&r__2);
    i__1 = numlin;
    for (ix = 1; ix <= i__1; ++ix) {
	...
	natr (&c_b76, &ypos, &c_b63, &c_b63, "AaMnZz09@#Q", 11L);
	ypos -= yincr;
/* L970: */
    }
