/* fort/04/02/04/01/p02.f -- translated by f2c (version of 22 July 1992  22:54:52).
...
#ifndef NO_PROTO
static real c_b61 = .4f;
static real c_b63 = .5f;
#else /* NO_PROTO */
static real c_b61 = (float).4;
static real c_b63 = (float).5;
#endif /* NO_PROTO */
...
#ifndef NO_PROTO
static real c_b126 = .25f;
static real c_b128 = .3f;
static real c_b140 = 1.f;
#else /* NO_PROTO */
static real c_b126 = (float).25;
static real c_b128 = (float).3;
static real c_b140 = (float)1.;
#endif /* NO_PROTO */
static integer c__0 = 0;
#ifndef NO_PROTO
static real c_b161 = 3.f;
#else /* NO_PROTO */
static real c_b161 = (float)3.;
#endif /* NO_PROTO */

/*  ********************************************************* */
/*  *                                                       * */
/*  *    TEST NUMBER: 04.02.04.01/02                        * */
...
#endif /* NO_PROTO */
{
...
#ifndef NO_PROTO
    extern /* Subroutine */ int inmsg_(char *, ftnlen), nosep (integer *);
#else /* NO_PROTO */
    extern /* Subroutine */ int inmsg_(), nosep ();
#endif /* NO_PROTO */
    ...
    extern /* Subroutine */ int initgl_(char *, ftnlen), xpopph_(integer *, 
	    ...
	    integer *), nstxfn (integer *), nstxpr (integer *), rnperm_(
	    integer *, integer *), numlab_(integer *, real *, real *, real *),
	     srtiar_(integer *, integer *), dilist_(char *, integer *, 
	    integer *, ftnlen), opfail_(void), tstasc_(char *, char *, char *,
	     ftnlen, ftnlen, ftnlen), chkinq_(char *, integer *, ftnlen), 
	    nschsp (real *), nstxci (integer *), nsplci (integer *), nschxp (
	    real *), dchpfv_(char *, integer *, integer *, ftnlen);
#else /* NO_PROTO */
    extern /* Subroutine */ int initgl_(), xpopph_(), setasf_(), setmsg_(), 
	    nsatal (), nsatch (), nseplb (), nstxfn (), nstxpr (), rnperm_(), 
	    numlab_(), srtiar_(), dilist_(), opfail_(), tstasc_(), chkinq_(), 
	    nschsp (), nstxci (), nsplci (), nschxp (), dchpfv_();
#endif /* NO_PROTO */
...
#ifndef NO_PROTO
    setmsg_("5 6 7 14 18 19", "Fonts 1 and 2 should both contain all the ASC"
	    "II special characters, in any precision.", 14L, 85L);
...
II special characters, in any precision.", 14L, 85L);
    ...
    nsatal (&c__1, &c__3);
#ifndef NO_PROTO
    r__1 = npcpwc * .5f * yincr;
#else /* NO_PROTO */
    r__1 = npcpwc * (float).5 * yincr;
#endif /* NO_PROTO */
    nsatch (&r__1);
...
/* Computing MIN */
		...
		numlab_(&numlft, &c_b61, &ypos, &yincr);
		i__1 = numlft;
		for (icx = 1; icx <= i__1; ++icx) {
		    iy = icx + grpbas;
		    natr (&c_b63, &ypos, &c_b3, &c_b3, ascch + (perm[iy - 1] 
			    - 1), 1L);
...
#ifndef NO_PROTO
    setmsg_("5 6 7 14 18 19", "Fonts 1 and 2 should both contain all the ASC"
	    "II uppercase letters, in any precision.", 14L, 84L);
#else /* NO_PROTO */
    setmsg_("5 6 7 14 18 19", "Fonts 1 and 2 should both contain all the ASC\
II uppercase letters, in any precision.", 14L, 84L);
#endif /* NO_PROTO */
...
/* Writing concatenation */
    ...
    tstasc_("ASCII UPPERCASE LETTERS: Which character is NOT an uppercase le"
	    "tter?", ch__1, "A", 68L, 27L, 1L);
    setmsg_("5 6 7 14 18 19", "Fonts 1 and 2 should both contain all the ASC"
	    "II lowercase letters, in any precision.", 14L, 84L);
#else /* NO_PROTO */
    tstasc_("ASCII UPPERCASE LETTERS: Which character is NOT an uppercase le\
tter?", ch__1, "A", 68L, 27L, 1L);
    setmsg_("5 6 7 14 18 19", "Fonts 1 and 2 should both contain all the ASC\
...
/* Writing concatenation */
    ...
    tstasc_("ASCII LOWERCASE LETTERS: Which character is NOT a lowercase let"
	    "ter?", ch__1, "A", 67L, 27L, 1L);
    setmsg_("5 6 7 14 18 19", "Fonts 1 and 2 should both contain all the ASC"
	    "II digit characters, in any precision.", 14L, 83L);
#else /* NO_PROTO */
    tstasc_("ASCII LOWERCASE LETTERS: Which character is NOT a lowercase let\
ter?", ch__1, "A", 67L, 27L, 1L);
    setmsg_("5 6 7 14 18 19", "Fonts 1 and 2 should both contain all the ASC\
...
/* Writing concatenation */
    ...
    tstasc_("ASCII DIGITS: Which character is NOT a digit?", ch__2, "A", 45L, 
	    11L, 1L);
/*  clear out everything */
    nsep (&c__1);
    ndellb (&c__1, &c__3);
#ifndef NO_PROTO
    setmsg_("5 6 8 14 18 19", "Text fonts 1 and 2 should be visually disting"
	    "uishable in any precision.", 14L, 71L);
#else /* NO_PROTO */
    setmsg_("5 6 8 14 18 19", "Text fonts 1 and 2 should be visually disting\
uishable in any precision.", 14L, 71L);
#endif /* NO_PROTO */
...
/*  perm = array to randomize order of 5 entries */
	rnperm_(&numlin, perm);
	r__1 = 1 - yincr;
	numlab_(&numlin, &c_b126, &r__1, &yincr);
	i__1 = numlin;
	for (ix = 1; ix <= i__1; ++ix) {
/*           display "Test" at location perm(ix) */
	    ...
	    natr (&c_b128, &ypos, &c_b3, &c_b3, "Test", 4L);
/* L320: */
	}
#ifndef NO_PROTO
...
L390:
#ifndef NO_PROTO
    setmsg_("5 6 9 10 11 23 27 33 49 50", "In text font 1, all character bod"
	    "ies affected by the same annotation text character height, chara"
	    "cter expansion factor, and character spacing should have the sam"
	    "e width.", 26L, 169L);
#else /* NO_PROTO */
    setmsg_("5 6 9 10 11 23 27 33 49 50", "In text font 1, all character bod\
ies affected by the same annotation text character height, character expansi\
on factor, and character spacing should have the same width.", 26L, 169L);
#endif /* NO_PROTO */
/*  Use <inquire text extent> to determine */
/*    ncwh = nominal character width/height ratio for font 1 */
    nqtxx (&dialog_1.specwt, &c__1, &c_b140, &c_b3, &c_b140, &c__0, &c__1, &
...
/* Writing concatenation */
	...
	nsatch (&r__1);
	nschxp (&chexp);
	natr (&xbase, &ypos, &c_b3, &c_b3, distrg, numch);
