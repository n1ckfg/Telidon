/* fort/04/02/04/02/p04.f -- translated by f2c (version of 22 July 1992  22:54:52).
...
#ifndef NO_PROTO
static real c_b47 = .04f;
#else /* NO_PROTO */
static real c_b47 = (float).04;
#endif /* NO_PROTO */
static integer c__2 = 2;
#ifndef NO_PROTO
static real c_b53 = 1.f;
static real c_b61 = .95f;
#else /* NO_PROTO */
static real c_b53 = (float)1.;
static real c_b61 = (float).95;
#endif /* NO_PROTO */
static integer c__3 = 3;
static integer c__5 = 5;
#ifndef NO_PROTO
static real c_b81 = .3f;
static real c_b82 = 3.f;
#else /* NO_PROTO */
...
#endif /* NO_PROTO */
{
    ...
    extern /* Subroutine */ int wcnpc_(real *, real *, real *, real *, real *)
	    , nclst (void), inmsg_(char *, ftnlen);
#else /* NO_PROTO */
    extern /* Subroutine */ int wcnpc_(), nclst (), inmsg_();
#endif /* NO_PROTO */
    ...
    extern /* Subroutine */ int dynpf_(char *, char *, ftnlen, ftnlen), 
	    nemst (integer *);
#else /* NO_PROTO */
    extern /* Subroutine */ int dynpf_(), nemst ();
#endif /* NO_PROTO */
    ...
    extern /* Subroutine */ int nopst (integer *), nqtxf (integer *, integer *
	    ...
	    nstxi (integer *), nsatp (integer *), unrep_(integer *, integer *,
	     integer *, char *, char *, ftnlen, ftnlen), nstxp (integer *), 
    ...
    extern /* Subroutine */ int nopst (), nqtxf (), nsvwi (), nstxi (), 
	    nsatp (), unrep_(), nstxp (), nstxr (), nqtxx ();
#endif /* NO_PROTO */
    ...
    extern /* Subroutine */ int initgl_(char *, ftnlen), xpopph_(integer *, 
	    ...
	    ftnlen, ftnlen), nstxal (integer *, integer *), nsatal (integer *,
	     integer *), nsiasf (integer *, integer *), nsplci (integer *), 
	    nsatch (real *), dchpfv_(char *, integer *, integer *, ftnlen), 
	    dilist_(char *, integer *, integer *, ftnlen);
#else /* NO_PROTO */
    extern /* Subroutine */ int initgl_(), xpopph_(), setasf_(), chkinq_(), 
	    setmsg_(), chkfnt_(), nstxal (), nsatal (), nsiasf (), nsplci (), 
	    nsatch (), dchpfv_(), dilist_();
#endif /* NO_PROTO */
...
/* L100: */
    ...
    setmsg_("1 3 4 11 12 13", "All positive text fonts should appear as spec"
...
#ifndef NO_PROTO
    chkfnt_(&lposx, lposfn, lpospr, &maxfnt, chstr, (logical*)&c__0, "B", 10L,
	     1L);
#else /* NO_PROTO */
    chkfnt_(&lposx, lposfn, lpospr, &maxfnt, chstr, &c__0, "B", 10L, 1L);
#endif /* NO_PROTO */
    dynpf_(msg, "Y", 300L, 1L);
    nemst (&c__102);
#ifndef NO_PROTO
    setmsg_("1 3 4 11 12 13 21 22", "All positive text fonts should appear a"
...
/*  call routine to handle display */
#ifndef NO_PROTO
    chkfnt_(&lposx, lposfn, lpospr, &maxfnt, chstr, (logical*)&c__1, "B", 10L,
	     1L);
#else /* NO_PROTO */
    chkfnt_(&lposx, lposfn, lpospr, &maxfnt, chstr, &c__1, "B", 10L, 1L);
#endif /* NO_PROTO */
    dynpf_(msg, "Y", 300L, 1L);
    nemst (&c__102);
/*  end_pos: */
L190:
    ...
    setmsg_("1 3 4 11 12 13", "All non-positive text fonts should appear as "
...
/*  call routine to handle display */
#ifndef NO_PROTO
    chkfnt_(&lnegx, lnegfn, lnegpr, &maxfnt, chstr, (logical*)&c__0, "B", 10L,
	     1L);
#else /* NO_PROTO */
    chkfnt_(&lnegx, lnegfn, lnegpr, &maxfnt, chstr, &c__0, "B", 10L, 1L);
#endif /* NO_PROTO */
    dynpf_(msg, "Y", 300L, 1L);
    nemst (&c__102);
#ifndef NO_PROTO
    setmsg_("1 3 4 11 12 13 21 22", "All non-positive text fonts should appe"
...
/*  call routine to handle display */
#ifndef NO_PROTO
    chkfnt_(&lnegx, lnegfn, lnegpr, &maxfnt, chstr, (logical*)&c__1, "B", 10L,
	     1L);
#else /* NO_PROTO */
    chkfnt_(&lnegx, lnegfn, lnegpr, &maxfnt, chstr, &c__1, "B", 10L, 1L);
#endif /* NO_PROTO */
    dynpf_(msg, "Y", 300L, 1L);
    nemst (&c__102);
/*  end_font_id: */
L390:
#ifndef NO_PROTO
    setmsg_("1 3 4 11 12 16 17 23 24 25 27 28", "In each text font, all char"
	    ...
	    "e the same height.", 32L, 173L);
#else /* NO_PROTO */
    setmsg_("1 3 4 11 12 16 17 23 24 25 27 28", "In each text font, all char\
acter bodies affected by the same annotation text character height, characte\
r expansion factor, and character spacing should have the same height.", 32L, 
	    173L);
#endif /* NO_PROTO */
...
/*  set up for labels */
    ...
    nstxi (&i__1);
    i__1 = numcol + 1;
    nstxr (&globnu_1.wkid, &i__1, &c__1, &c__2, &c_b53, &c_b4, &c__1);
...
/*  display vertical annotation text */
    xpos = xleft;
#ifndef NO_PROTO
    chsp = .4f;
#else /* NO_PROTO */
    ...
    ytop = (float).85;
#endif /* NO_PROTO */
/*  try to paint annotation text and polylines different colors */
    nsiasf (&c__2, &c__1);
    nsplci (&c__2);
    i__1 = numcol;
    for (ix = 1; ix <= i__1; ++ix) {
...
/*          requested character height)  for txfnt */
	nqtxx (&dialog_1.specwt, &txfnt, &c_b53, &c_b4, &c_b53, &c__3, &c__1, 
...
/*  display distrg at location ix */
	...
	natr (&xpos, &ytop, &c_b4, &c_b4, distrg, numch);
...
#ifndef NO_PROTO
    setmsg_("1 3 4 11 12 14 19", "There should be an implementation dependen"
...
/*  check font 1 - if no good, fail and skip out */
    unrep_(&c__1, &unr1sz, unr1ls, "B", result, 1L, 1L);
    if (*result == 'F') {
	fail_();
	goto L890;
    }
...
/*  do_f2: */
L850:
    unrep_(&c__2, &unr2sz, unr2ls, "B", result, 1L, 1L);
    L__1 = *result == 'P';
    ifpf_(&L__1);
/*  end_unrep: */
L890:
