/* fort/09/02/04/p02.f -- translated by f2c (version of 22 July 1992  22:54:52).
...
/*  ********************************************************* */
/*  *                                                       * */
/*  *    TEST NUMBER: 09.02.04/02                           * */
/*  *    TEST TITLE : Error indicator = 152                 * */
/*  *                                                       * */
...
/*   viewtype */
    initgl_("09.02.04/02", 11L);
    xpopph_(&globnu_1.errfil, &globnu_1.memun);
#ifndef NO_PROTO
    s_copy(errchr_1.curcon, "the specified viewport is invalid: XMIN>=XMAX, "
	    "YMIN>=YMAX, or ZMIN>ZMAX", 200L, 71L);
#else /* NO_PROTO */
    s_copy(errchr_1.curcon, "the specified viewport is invalid: XMIN>=XMAX, \
YMIN>=YMAX, or ZMIN>ZMAX", 200L, 71L);
#endif /* NO_PROTO */
...
/*  XMIN > XMAX */
    ...
    setrvs_("0.4082, 0.2640, 0.01339, 0.7457, 0.1395, 0.30036", pjvplm, &asiz,
	     48L);
    pjtype = 0;
    ...
    revmm3_(vwwnlm, pjvplm, &pjtype, prp, &prp[1], &prp[2], &vpd, &bpd, &fpd, 
	    &errind, ra44);
/*  YMIN > YMAX */
    setrvs_("0.2640, 0.4082, 0.7357, 0.01339, 0.1395, 0.1395", pjvplm, &asiz, 
	    47L);
    revmm3_(vwwnlm, pjvplm, &pjtype, prp, &prp[1], &prp[2], &vpd, &bpd, &fpd, 
	    &errind, ra44);
/*  XMIN = XMAX */
    setrvs_("-62.2236, -11.39577, -60.530, 24.255", vwwnlm, &asiz, 36L);
    setrvs_("0.1063, 0.1063, 0.1001, 0.1539, 0.2469, 0.8826", pjvplm, &asiz, 
	    46L);
    pjtype = 1;
    ...
    revmm3_(vwwnlm, pjvplm, &pjtype, prp, &prp[1], &prp[2], &vpd, &bpd, &fpd, 
	    &errind, ra44);
/*  ZMIN > ZMAX */
    setrvs_("0.1063, 0.2224, 0.1001, 0.1539, 0.8826, 0.2469", pjvplm, &asiz, 
	    46L);
    revmm3_(vwwnlm, pjvplm, &pjtype, prp, &prp[1], &prp[2], &vpd, &bpd, &fpd, 
	    &errind, ra44);
/*  XMIN > XMAX */
    setrvs_("-23.32, 3.56E4, 2.29, 2.295", vwwnlm, &asiz, 27L);
    setrvs_("0.736, 0.298, 0.22, 0.89", pjvpl2, &asiz, 24L);
    revmm_(vwwnlm, pjvpl2, &errind, ra33);
/*  YMIN = YMAX */
    setrvs_("0.298, 0.736, 0.89, 0.89", pjvpl2, &asiz, 24L);
    revmm_(vwwnlm, pjvpl2, &errind, ra33);
    endit_();
#ifndef NO_PROTO
    return 0;
