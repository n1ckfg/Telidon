C  *********************************************************
C  *                                                       *
C  *    TEST NUMBER: 04.02.03.01/07                        *
C  *    TEST TITLE : Text color index                      *
C  *                                                       *
...
C  Declare program-specific variables
      ...
      REAL       RAD, CENTX,CENTY, PI
      PARAMETER (CSIZ = 6, PI = 3.14159265)
      REAL       BCKCOL(CSIZ), FORCOL(CSIZ), NEWCOL(CSIZ)
      REAL       YLOC,YINCR, XA(9),YA(9), CHHT


      CALL INITGL ('04.02.03.01/07')
...
C  use individual attributes
      ...
      CALL SETMSG ('11 46 47 48', 'A defined text color index '     //
...
C  draw text line #ix with text colour index = txcol(ix)
         CALL PTX (0.2, YLOC, 'Defined text color')
         YLOC  = YLOC-YINCR
2100  CONTINUE
...
C  set entry txcol(visdx) to forcol
      ...
      CALL DCHPF ('DEFINED TEXT COLOR INDICES: Which text line ' //
     1            'is visible?', NUMLIN, VISDX, PERM)

C  clear out last display from structure
      CALL PEMST (102)

      CALL SETMSG ('11 46 47 49', 'An undefined text color index '   //
     1             'should cause entry number 1 in the color table ' //
     2             'to be used when rendering a text primitive.')
...
C  for each color entry - set to background color and draw text string
      DO 2300 IX = 1,NUMLIN
         COLEL = PERM(IX)
         IF (COLEL .LE. 3) THEN
...
C  randomly pick and set a precision, to ensure no interaction
         CALL PSTXPR (RNDINT (PSTRP, PSTRKP))
         CALL PTX (.1, YLOC, 'Undefined color')
         YLOC  = YLOC-YINCR
2300  CONTINUE

      CALL DCHPFV ('UNDEFINED TEXT COLOR INDICES: How many text ' //
     1             'lines are the same color as the star?', 12, NUMLIN)
