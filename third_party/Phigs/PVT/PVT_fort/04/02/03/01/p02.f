...
C  Note: some Fortran compilers running under UNIX treat the
C  backslash character within source code as an escape character,
C  and therefore reject the following DATA statement.  Two possible
C  fixes are: 1) set a compiler option to accept the source code as
C  is, or 2) use a double-backslash in the DATA statement.
...
C  set up array for ASCII names:
      ...
      CALL SETMSG ('11 13 14 21 26 27', 'Fonts 1 and 2 should both ' //
...
295   CONTINUE
     ...
     1             NONMEM(IX:IX) // DIGIT, 'T')

C  clear out everything
      CALL PSEP (1)
      CALL PDELLB (1,3)

      CALL SETMSG ('11 13 15 21 26 27', 'Text fonts 1 and 2 should ' //
     1             'be visually distinguishable in any precision.')
...
390   CONTINUE
      ...
      CALL SETMSG ('11 13 16 17 18 31 36 42 59 60', 'In text font ' //
     1             '1, all character bodies affected by the same '  //
     2             'character height, character expansion factor, ' //
     3             'and character spacing should have the same width.')
