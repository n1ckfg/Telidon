...
C text precision
      ...
      INTEGER     PICSTR, TXCI, ITRIM, NUMCOL, NGCOL, RNDINT
...
100   CONTINUE
     ...
     2      'corresponding description in the ISO register?'
      ...
      CALL SETMSG ('11 13 19 21', 'All positive text fonts should ' //
     1             'appear as specified in the ISO register.')
      CALL INMSG ('SR 19 is indirectly confirmed in the following '  //
...
C  call routine to handle display
      CALL CHKFNT (LPOSX,LPOSFN,LPOSPR, MAXFNT, CHSTR, .FALSE., 'T')
      CALL DYNPF (MSG, 'y')
      CALL PEMST (102)

      CALL SETMSG ('11 13 19 21 29 30', 'All positive text fonts ' //
     1             'should appear as specified in the ISO '        //
     2             'register, even when less precision is '        //
     3             'requested than is available for the font.')

C  call routine to handle display
      CALL CHKFNT (LPOSX,LPOSFN,LPOSPR, MAXFNT, CHSTR, .TRUE., 'T')
      CALL DYNPF (MSG, 'y')
      CALL PEMST (102)
...
190   CONTINUE
     ...
     1      'workstation types which support that identifier.'
      ...
      CALL SETMSG ('11 13 19 21', 'All non-positive text fonts ' //
     1             'should be workstation independent and '      //
     2             'appear as specified in the implementor''s '  //
     3             'documentation.')

C  call routine to handle display
      CALL CHKFNT (LNEGX,LNEGFN,LNEGPR, MAXFNT, CHSTR, .FALSE., 'T')
      CALL DYNPF (MSG, 'y')
      CALL PEMST (102)

      CALL SETMSG ('11 13 19 21 29 30', 'All non-positive text '  //
     1             'fonts should be workstation independent and ' //
     2             'appear as specified in the implementor''s '   //
     3             'documentation, even when less precision is '  //
     4             'requested than is available for the font.')

C  call routine to handle display
      CALL CHKFNT (LNEGX,LNEGFN,LNEGPR, MAXFNT, CHSTR, .TRUE., 'T')
      CALL DYNPF (MSG, 'y')
      CALL PEMST (102)
...
290   CONTINUE
      ...
      IF (LNEGX .LE. MAXFNT) GOTO 390

      CALL SETMSG ('21', 'All reported implementor-defined text '  //
     1             'fonts available for a given workstation type ' //
     2             'should be documented by the implementor.')

C  list all reported implementor-defined text fonts in lnegfn
      CALL SRTIAR (LNEGX, LNEGFN)
      CHFNTS = ' '
      WRITE (CHFNTS, '(200I5)') (LNEGFN(IX), IX=1,LNEGX)
      CALL DYNPF ('COMPLETE DOCUMENTATION FOR IMPLEMENTOR TEXT '  //
     1            'FONTS: Are all these fonts documented by the ' //
     2            'implementor:' // CHFNTS(1:ITRIM(CHFNTS)), 'y')

C  end_font_id:
390   CONTINUE

      CALL SETMSG ('11 13 16 17 31 36 42 59', 'In each text font, ' //
     1             'all character bodies affected by the same '     //
     2             'character height, character expansion factor, ' //
     3             'and character spacing should have the same '    //
     4             'height.')
...
600   CONTINUE
     ...
     2             'the separators?', NUMCOL, NGCOL)
      ...
      CALL SETMSG ('1 11 14 16 17 31 58 59 75', 'Character height '   //
     1             'magnitude should specify the nominal height, in ' //
     2             'TLC, from baseline to capline, of all capital '   //
     3             'letters within a font.')

      CALL DILIST ('CHARACTER HEIGHT FOR SIZE OF CAPITAL LETTERS: ' //
     1             'Enter list of font identifiers available in '   //
     2             'stroke precision, other than 1 and 2, which '   //
     3             'support alphabetic characters (n if none).',
     4             ALFSIZ, ALFONT)
C  add 1 and 2
      ALFONT(ALFSIZ+1) = 1
      ALFONT(ALFSIZ+2) = 2
...
C  display and label str, using chht and curfnt
         CALL PSTXFN (CURFNT)
         CALL PTX (LBOX+0.1, YPOS, STR)
         YPOS = YPOS - YINCR
C  next ix
700   CONTINUE

      CALL DCHPFV ('CHARACTER HEIGHT FOR SIZE OF CAPITAL LETTERS: ' //
     1             'In which line do the upper-case letters NOT '   //
     2             'fit exactly between the dashed baseline and '   //
     3             'the dotted capline?', NUMLIN, NGLINE)
      CALL PEMST (102)
     ...
     3             UNR2SZ, UNR2LS)
      ...
      IF (UNR1SZ.EQ.0 .AND. UNR2SZ.EQ.0) THEN
         CALL INMSG ('Cannot test SR20, because there are no '     //
     1               'unrepresentable character codes in fonts 1 ' //
     2               'or 2.')
         GOTO 890
      ENDIF

      CALL SETMSG ('13 14 20', 'There should be a implementation ' //
...
C  check font 1 - if no good, fail and skip out
      CALL UNREP (1, UNR1SZ, UNR1LS, 'T', RESULT)
      IF (RESULT .EQ. 'F') THEN
         CALL FAIL
         GOTO 890
      ENDIF
...
C  do_f2:
850   CONTINUE
      CALL UNREP (2, UNR2SZ, UNR2LS, 'T', RESULT)
      CALL IFPF (RESULT .EQ. 'P')

C  end_unrep:
890   CONTINUE

      CALL SETMSG ('11 13 15 22 27', 'If text display is requested ' //
     1             'with an unavailable font or precision, font=1 '  //
     2             'and precision=STRING should be used.')
...
C  allow room within picture square = (x-space - 0.1) / #characters
      CALL PSCHH (MIN (YINCR/2, 0.5/11))
      DO 970 IX = 1,NUMLIN
         CALL PSTXFN (LUNSFN(PERM(IX)))
         CALL PSTXPR (LUNSPR(PERM(IX)))
         CALL PTX (.3, YPOS, 'AaMnZz09@#Q')
         YPOS = YPOS-YINCR
970   CONTINUE
