...
C  txstr = text string to be used for test = "Widget!?"
      TXSTR = 'Widget!?'
...
C
C       hor-align  vert-align  text path
...
C  calculate txpos = text position, so as to fit text
...
C  if ix=6 then change txpos to generate incorrect result
         ...
         ENDIF
C  set text alignment = algnh(ix), algnv(ix)
         CALL PSTXAL (ALGNH(IX), ALGNV(IX))
C  set text path = algntp(ix)
         CALL PSTXP (ALGNTP(IX))
C  draw text at txpos
         CALL PTX (TXPOSX,TXPOSY, TXSTR)
300   CONTINUE

      CALL SETMSG ('16 27 31 65 66 67 72 73 74 75', 'Explicit text '  //
     1             'alignment attributes should affect the position ' //
     2             'of a horizontal text rectangle in accordance '    //
     3             'with the values returned by <inquire text extent>.')
      CALL DCHPFV ('EXPLICIT ALIGNMENT FOR HORIZONTAL TEXT: Which ' //
     1             'text primitive is NOT aligned within the '      //
     2             'dotted text extent rectangle on the dashed '    //
     3             'baseline?', NUMLIN, PERM(NUMLIN))
      CALL PEMST (102)
...
C  draw and label 5 identical horizontal text rectangles,
...
C  if ix=5 then change txpos to generate incorrect result
         IF (IX .EQ. NUMLIN) THEN
            TXPOSX = TXPOSX + 0.1 * (RNDINT(0,1) - 0.5)
         ENDIF
C  set text alignment = ralgnh(ix), ralgnv(ix)
         CALL PSTXAL (RALGNH(IX), RALGNV(IX))
C  set text path = algntp(ix)
         CALL PSTXP (ALGNTP(IX))
C  draw text at txpos
         CALL PTX (TXPOSX,TXPOSY, TXSTR)
600   CONTINUE
      ...
      CALL SETMSG ('16 27 31 66 67 72 73 74 75 76', 'Implicit text '  //
     1             'alignment attributes should affect the position ' //
     2             'of a horizontal text rectangle in accordance '    //
     3             'with the values returned by <inquire text '       //
     4             'extent>.')
      CALL DCHPFV ('IMPLICIT ALIGNMENT FOR HORIZONTAL TEXT: Which ' //
     1             'text primitive is NOT aligned within the '      //
     2             'dotted text extent rectangle on the dashed '    //
     3             'baseline?', NUMLIN, PERM(NUMLIN))
      CALL PEMST (102)
...
C  draw and label 6 identical vertical text rectangles,
C    using dotted lines, distinct color,
...
C  char height for text under test
      CALL PSCHH (CHHT)
...
800   CONTINUE
      ...
      CALL SETMSG ('16 27 31 65 66 67 72 73 74 75', 'Explicit text '  //
     1             'alignment attributes should affect the position ' //
     2             'of a vertical text rectangle in accordance with ' //
     3             'the values returned by <inquire text extent>.')
      CALL DCHPFV ('EXPLICIT ALIGNMENT FOR VERTICAL TEXT: Which ' //
     1             'text primitive is NOT aligned within the '    //
     2             'dotted text extent rectangle on the dashed '  //
     3             'centerline?', NUMLIN, PERM(NUMLIN))
      CALL PEMST (102)
...
C  draw and label 5 identical vertical text rectangles,
C    using dotted lines, distinct color,
...
1000  CONTINUE
     ...
     2             'of a vertical text rectangle in accordance with ' //
     3             'the values returned by <inquire text extent>.')
      CALL DCHPFV ('IMPLICIT ALIGNMENT FOR VERTICAL TEXT: Which ' //
     1             'text primitive is NOT aligned within the '    //
     2             'dotted text extent rectangle on the dashed '  //
     3             'centerline?', NUMLIN, PERM(NUMLIN))
      CALL PEMST (102)
