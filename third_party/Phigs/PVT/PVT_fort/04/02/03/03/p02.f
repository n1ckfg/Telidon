C  *********************************************************
C  *                                                       *
C  *    TEST NUMBER: 04.02.03.03/02                        *
C  *    TEST TITLE : Text rectangle size and concatenation *
C  *                 point for horizontal text orientation *
C  *                                                       *
...
C text precision
     ...
     1            MNCHH, MXCHH, MNCHXP, MXCHXP, NOMCW,
     2            NOMW, NOMH, RDUM1,RDUM2
      ...
      CALL INITGL ('04.02.03.03/02')

C open PHIGS
      CALL XPOPPH (ERRFIL, MEMUN)
C  open primary workstation
...
C  call CHTXRC with: nomw, nomh, specwt, font, str unchanged
     ...
     1             TXOR, CHXP, CHSP, CHH, DIDTST, RECTOK, CCPOK)
      ...
      CALL SETMSG ('6 13 15', 'The width of a text rectangle with '   //
     1             'zero character spacing and a horizontal text '    //
     2             'path should expand in proportion to the '         //
     3             'character expansion factor.')
      CALL IFPF (RECTOK)

      CALL SETMSG ('6 10 13 17', 'With zero character spacing and a ' //
     1             'horizontal text path, the concatenation point '   //
     2             'of a text rectangle should reflect its '          //
...
C  call CHTXRC with: nomw, nomh, specwt, font, str unchanged
     ...
     1             TXOR, CHXP, CHSP, CHH, DIDTST, RECTOK, CCPOK)
      ...
      CALL SETMSG ('6 13 14 15', 'The width of a text rectangle '  //
     1             'with zero character spacing and a horizontal ' //
     2             'text path should expand in proportion to the ' //
     ...
     1             'and a horizontal text path, the concatenation ' //
     2             'point of a text rectangle should reflect its '  //
     3             'expansion in proportion to the character '      //
...
C  call CHTXRC with: nomw, nomh, specwt, font, str unchanged
     ...
     1             'with zero character spacing and a horizontal ' //
     2             'text path should expand in proportion to the ' //
     3             'character expansion factor, even when it is '  //
     4             'less than the minimum for the workstation type.')
      CALL IFPF (RECTOK)
...
C  call CHTXRC with: nomw, nomh, specwt, font, str unchanged
     ...
     1             TXOR, CHXP, CHSP, CHH, DIDTST, RECTOK, CCPOK)
      ...
      CALL SETMSG ('8 9 13 15', 'The size of a text rectangle with '  //
     1             'character spacing = 0, character expansion '      //
     2             'factor = 1, and a horizontal text path should '   //
     3             'expand in proportion to the character height.')
      CALL IFPF (RECTOK)

      CALL SETMSG ('8 9 10 13 17', 'With character spacing = 0, '     //
     1             'character expansion factor = 1, and a '           //
     2             'horizontal text path, the concatenation point '   //
     3             'of a text rectangle should reflect its '          //
     4             'expansion in proportion to the character height.')
      CALL IFPF (CCPOK)
...
C  call CHTXRC with: nomw, nomh, specwt, font, str unchanged
     ...
     1             TXOR, CHXP, CHSP, CHH, DIDTST, RECTOK, CCPOK)
      ...
      CALL SETMSG ('8 9 13 14 15', 'The size of a text rectangle '    //
     ...
     2             'horizontal text path, the concatenation point '   //
     3             'of a text rectangle should reflect its '          //
     4             'expansion in proportion to the character '        //
     5             'height, even when it exceeds the maximum for '    //
     6             'the workstation type.')
      CALL IFPF (CCPOK)
...
C  call CHTXRC with: nomw, nomh, specwt, font, str unchanged
     ...
     3             'of a text rectangle should reflect its '          //
     4             'expansion in proportion to the character '        //
     5             'height, even when it is less than the minimum '   //
     6             'for the workstation type.')
      CALL IFPF (CCPOK)
...
C  *** *** ***   character spacing   *** *** ***
C
C  call CHTXRC with: nomw, nomh, pecwt, font, str unchanged
      TXOR = HRIZNT
      CHXP = 1.0
      CHSP = 0.3
      CHH  = 1.0
     ...
     1             TXOR, CHXP, CHSP, CHH, DIDTST, RECTOK, CCPOK)
      ...
      CALL SETMSG ('7 13 15', 'The width of a text rectangle with '   //
     1             'character expansion factor = 1, character '       //
     2             'height = 1, and a horizontal text path should '   //
     3             'increase by the character spacing times one '     //
     4             'less than the number of characters.')
      CALL IFPF (RECTOK)

      CALL SETMSG ('7 10 13 17 18', 'With character expansion '     //
     1             'factor = 1, character height = 1, and a '       //
     2             'horizontal text path, the concatenation point ' //
     3             'of a text rectangle should reflect its '        //
     4             'increased size due to character spacing.')
      CALL IFPF (CCPOK)
...
C  call CHTXRC with: nomw, nomh, specwt, font, str unchanged
     ...
     1             TXOR, CHXP, CHSP, CHH, DIDTST, RECTOK, CCPOK)
      ...
      CALL SETMSG ('6 7 8 9 13 15', 'The size of a text rectangle '   //
     1             'with a horizontal text path should reflect the '  //
     ...
     2             'and character height, and a horizontal text '     //
     3             'path, the concatenation point of a text '         //
     4             'rectangle should reflect its increased size.')
      CALL IFPF (CCPOK)
...
C  call CHTXRC with: nomw, nomh, specwt, font, str unchanged
     ...
     2             'and character height, and a horizontal text '     //
     3             'path, the concatenation point of a text '         //
     4             'rectangle should reflect its decreased size.')
      CALL IFPF (CCPOK)
...
C  call CHTXRC with: nomw, nomh, specwt, font, str unchanged
     ...
     2             'spacing, and a horizontal text path, the '        //
...
C  call CHTXRC with: nomw, nomh, specwt, font, str unchanged
     ...
     2             'horizontal text path, the concatenation point '   //
     3             'of a text rectangle should reflect its altered '  //
     4             'size.')
      CALL IFPF (CCPOK)
...
C  <Inquire text extent> with:
     ...
     1            STR(1:4), ERRIND, TXEXRX, TXEXRY, RDUM1, RDUM2)
      CALL CHKINQ ('pqtxx',ERRIND)
      NOMCW  = ABS (TXEXRX(2) - TXEXRX(1)) / 4
      NOMH   = ABS (TXEXRY(2) - TXEXRY(1))

C  call CHMONO with: nomcw, nomh, specwt, font, str unchanged
      TXOR = HRIZNT
      CHXP =  6.35
      CHSP = -8.51 * ABS(CHXP * NOMCW)
      CHH  =  -.021
      CALL CHMONO (NOMCW, NOMH, SPECWT, FONT, STR(1:4),
...
C  call CHMONO with: nomcw, nomh, specwt, font, str unchanged
     ...
     3             'magnitude than the expanded width of a single '   //
     4             'character.')
