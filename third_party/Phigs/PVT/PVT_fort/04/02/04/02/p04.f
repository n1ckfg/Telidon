...
C text precision
      ...
      REAL        TXRX(2),TXRY(2), NPCX, NPCY, NPCPWC, Z
...
100   CONTINUE
     ...
     2      'corresponding description in the ISO register?'
      ...
      CALL SETMSG ('1 3 4 11 12 13', 'All positive text fonts ' //
     1             'should appear as specified in the ISO register.')
 
C  call routine to handle display
      CALL CHKFNT (LPOSX,LPOSFN,LPOSPR, MAXFNT, CHSTR, .FALSE.,'B')
      CALL DYNPF (MSG, 'Y')
      CALL PEMST (102)
 
      CALL SETMSG ('1 3 4 11 12 13 21 22', 'All positive text fonts ' //
     2             'should appear as specified in the ISO register, ' //
     3             'even when less precision is requested than is '   //
     4             'available for the font.')
 
C  call routine to handle display
      CALL CHKFNT (LPOSX,LPOSFN,LPOSPR, MAXFNT, CHSTR, .TRUE.,'B')
      CALL DYNPF (MSG, 'Y')
      CALL PEMST (102)
...
190   CONTINUE
     ...
     2       'description in the implementor documentation? '
      ...
      CALL SETMSG ('1 3 4 11 12 13', 'All non-positive text fonts ' //
     1             'should appear as specified in the '             //
     2             'implementor''s documentation.')
 
C  call routine to handle display
      CALL CHKFNT (LNEGX,LNEGFN,LNEGPR, MAXFNT, CHSTR,.FALSE.,'B')
      CALL DYNPF (MSG, 'Y')
      CALL PEMST (102)
 
      CALL SETMSG ('1 3 4 11 12 13 21 22','All non-positive text ' //
...
C  call routine to handle display
      CALL CHKFNT (LNEGX,LNEGFN,LNEGPR, MAXFNT, CHSTR,.TRUE.,'B')
      CALL DYNPF (MSG, 'Y')
      CALL PEMST (102)
...
390   CONTINUE
      ...
      CALL SETMSG ('1 3 4 11 12 16 17 23 24 25 27 28', 'In each ' //
     2             'text font, all character bodies affected by ' //
     3             'the same annotation text character height, '  //
     4             'character expansion factor, and character '   //
     5             'spacing should have the same height.')
...
C  display vertical annotation text
      XPOS = XLEFT
      CHSP = 0.4
      CALL PSATP  (PDOWN)
      CALL PSATAL (PALEFT,PATOP)
      YTOP = 0.85
C  try to paint annotation text and polylines different colors
...
C  display distrg at location ix
         ...
         CALL PATR (XPOS, YTOP, Z, Z, DISTRG(:NUMCH))
...
600   CONTINUE
     ...
     2               'or 2.')
      ...
      CALL SETMSG ('1 3 4 11 12 14 19', 'There should be an '  //
...
C  check font 1 - if no good, fail and skip out
      CALL UNREP (1, UNR1SZ, UNR1LS, 'B', RESULT)
      IF (RESULT .EQ. 'F') THEN
         CALL FAIL
         GOTO 890
      ENDIF
...
C  do_f2:
850   CONTINUE
      CALL UNREP (2, UNR2SZ, UNR2LS, 'B', RESULT)
