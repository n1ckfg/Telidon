...
C text alignment vertical
     ...
     1           PABASE = 4, PABOTT = 5)
      ...
      INTEGER    FONTA,FONTB, NUMLIN,NGLINE, NUMCH,NUMNAR, TXDIF
 
      REAL       YPOS,YINCR, TXRX(2), TXRY(2), RDUM1,RDUM2
      REAL       NCWH, CHHT,CHEXP,CHSP, CHSIZ, RNDRL, XPOS, XBASE
      REAL       OFFACT, XA(5), YA(5), NPCX, NPCY, NPCPWC, Z
      PARAMETER  (Z = 0.0)
...
C  set up array for ASCII names:
      ...
      CALL SETMSG ('1 3 4 10 11 12 13 14', 'Fonts 1 and 2 should '   //
...
C  clear out everything
     ...
     1             'an uppercase letter?', NONMEM(IX:IX) // UPPER, 'B')
...
395   CONTINUE
     ...
     1             'a lowercase letter?', UPPER(IX:IX) // LOWER, 'B')
     ...
     1             NONMEM(IX:IX) // DIGIT, 'B')
 
C  clear out everything
      CALL PSEP (1)
      CALL PDELLB (1,3)
 
      CALL SETMSG ('1 3 4 10 11 12 13 15', 'Text fonts 1 and 2 ' //
...
690   CONTINUE
      ...
      CALL SETMSG ('1 3 4 11 12 16 17 18 23 24 25 27 28', 'In text ' //
     2             'font 1, all character bodies affected by the '   //
     3             'same annotation text character height, '         //
     4             'character expansion factor, and character '      //
     5             'spacing should have the same width.')
...
C  annotation text alignment = LEFT,BASE
      CALL PSATAL (PALEFT,PAHALF)
