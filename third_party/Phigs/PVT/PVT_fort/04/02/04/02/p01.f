...
C text alignment vertical
     ...
     1           PABASE = 4, PABOTT = 5)
      ...
      REAL      Z, YLOC, YINCR, YBOT,YTOP, PDEX, PDSP, NPCX,NPCY,NPCPWC
...
C  location of #7
      ...
      CALL SETMSG ('1 4 8 9', 'Immediately after <open '             //
     ...
     3             'corresponding predefined text bundles of the WDT.')
      ...
      CALL DRWATX (NUMBUN, BUNDIS, BUNDIF)
C  mark end of bundles 1-6
      CALL PLB (2)
 
      CALL DCHPFV ('WORKSTATION DESCRIPTION AND STATE LIST TABLES, '  //
     1             'INDICES 1-6: Which pair of annotation text '      //
     2             'relative primitives does NOT match?',NUMBUN, BUNDIF)
...
C  bundif = randomly selected bundle to be shown incorrectly
      ...
      CALL DCHPFV ('WORKSTATION DESCRIPTION AND STATE LIST TABLES, '  //
     1             'INDICES > 6: Which pair of annotation text '      //
     2             'relative primitives does NOT match?', NUMBUN,BUNDIF)
...
C *** *** *** ***  predefined bundle descriptions
      ...
      CALL SETMSG ('1 4 8', 'The appearance of predefined text ' //
...
600   CONTINUE
     ...
     1        'each annotation text relative primitive agree with ' //
     2        'the displayed aspect values for that bundle?')) THEN
