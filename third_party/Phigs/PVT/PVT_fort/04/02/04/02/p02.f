...
C text precision
      ...
      REAL      YLOC, YINCR, X(2), Y(2), NPCX, NPCY, NPCPWC, Z
      PARAMETER (Z = 0.0)
 
      CALL INITGL ('04.02.04.02/02')
...
C  mark start of text bundles
      ...
      CALL SETMSG ('1 2 3 4', 'A defined text index should cause '    //
     1             'the addressed entry in the bundle table to be '   //
     2             'used when rendering an annotation text relative ' //
     3             'primitive.')
...
C  mark end of linetypes
      ...
      CALL DCHPFV ('DEFINED TEXT INDICES: Which annotation text ' //
     1             'relative primitive is different?', BUNDIS, BUNDIF)
 
C  clear out last display from structure
      CALL PSEP (1)
      CALL PDELLB (1,2)
 
      CALL SETMSG ('1 2 3 5', 'An undefined text index should '     //
     1             'cause bundle number 1 in the text bundle '      //
     2             'table to be used when rendering an annotation ' //
     3             'text relative primitive.')
...
C     random integer from 0 to 4
      ...
      CALL PSATCH (0.05 * NPCPWC)
      CALL PSATAL (PACENT, PABASE)
C  draw text with bundle #1
      CALL PSTXI (1)
      CALL PATR (0.5, 0.9, Z, Z, 'Test text')
      X(1) = 0.3
      X(2) = 0.7
      Y(1) = 0.8
      Y(2) = 0.8
...
C    three annotation text primitives with bundles u1,u2,u3,
C    explct annotation text primitive with bundle #1
      ...
      DO 500 IX = 1,BUNDIS
         ...
         ELSE
            CALL PSTXI (1)
         ENDIF
         CALL PATR (0.5, YLOC, Z, Z, 'Test text')
         YLOC  = YLOC-YINCR
500   CONTINUE
 
      CALL DCHPFV ('UNDEFINED TEXT INDICES: How many of the '       //
     1             'annotation text relative primitives below the ' //
     2             'line have the same text attributes as the '     //
     3             'reference annotation text primitive above it?',
     4             12, BUNDIS)
