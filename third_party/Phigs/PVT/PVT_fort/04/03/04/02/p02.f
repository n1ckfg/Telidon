C  *********************************************************
C  *                                                       *
C  *    TEST NUMBER: 04.03.04.02/02                        *
C  *    TEST TITLE : Invisibility filter - setting and     *
C  *                 inquiring                             *
C  *                                                       *
...
C open PHIGS
      ...
      CALL SETMSG ('6 7', 'Immediately after opening the '           //
     1             'workstation, the invisibility filter inclusion ' //
     2             'and exclusion sets should be null.')

C  <inquire invisibility filter>
      CALL PQIVFT (WKID, SETSIZ,SETSIZ,
...
C                          exclusion = null)
     ...
     2           EXQSIZ .EQ. 0)
      ...
      CALL SETMSG ('5 6 10', 'Invisibility filter should be ' //
     1             'reported as it was set for any set of '   //
     2             'names drawn from the accessible range.')
...
C                          exclusion set = orig_inclusion)
     ...
     4           SETEQ (EXCSIZ, EXCSET, EXQSET) )
      ...
      CALL SETMSG ('5 6', 'Invisibility filter should be reported ' //
...
C                          exclusion set = new_exclusion)
     ...
     4           SETEQ (EXCSIZ, EXCSET, EXQSET) )
      ...
      CALL SETMSG ('5 6 11', 'Invisibility filter should be capable ' //
...
C    nanams = number of available names in namesets
     ...
     1            IDUM4,IDUM5,IDUM6,IDUM7)
      ...
      CALL SETMSG ('5 6 12', 'Invisibility filter should be capable ' //
...
C                          exclusion set = new_exclusion)
     ...
     4           SETEQ (EXCSIZ, EXCSET, EXQSET) )
      ...
      CALL SETMSG ('5 6 12 13', 'Invisibility filter should contain ' //
     1             'only the valid names as set; invalid names '      //
     2             'specified by <set invisibility filter> should '   //
     3             'be ignored.')
...
C  <inquire invisibility filter>
     ...
     4           SETEQ (EXCSIZ, EXCSET, EXQSET) )
      ...
      CALL SETMSG ('6 7', 'Immediately after re-opening the '         //
     1             'workstation, the invisibility filter inclusion '  //
     2             'and exclusion sets should be null.')
