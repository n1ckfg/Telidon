C  *********************************************************
C  *                                                       *
C  *    TEST NUMBER: 09.02.04/01                           *
C  *    TEST TITLE : Error indicator = 151                 *
C  *                                                       *
...
C   viewtype
      ...
      CALL INITGL ('09.02.04/01')

      CALL XPOPPH (ERRFIL, MEMUN)

      CURCON = 'the specified window is invalid: ' //
     1         'UMIN>=UMAX or VMIN>=VMAX'
      ERRSRS = '1'
      CALL SETVS ('151', EXPERR, EXPSIZ)

C  UMIN > UMAX
      CALL SETRVS ('-80.95, -462.27, -4891.85, -94.78', VWWNLM, ASIZ)
...
C  VMIN = VMAX
      CALL SETRVS ('-462.27, -80.95, -94.78, -94.78', VWWNLM, ASIZ)
      CALL REVMM3 (VWWNLM, PJVPLM, PJTYPE, PRP(1), PRP(2), PRP(3),
     1             VPD, BPD, FPD, ERRIND, RA44)

C  VMIN > VMAX
      CALL SETRVS ('-62.2236, -11.39577, 24.255, -60.530', VWWNLM, ASIZ)
...
C  UMIN = UMAX
      CALL SETRVS ('-62.2236, -62.2236, -60.530, 24.255',
     1             VWWNLM, ASIZ)
      CALL REVMM3 (VWWNLM, PJVPLM, PJTYPE, PRP(1), PRP(2), PRP(3),
     1             VPD, BPD, FPD, ERRIND, RA44)

C  VMIN > VMAX
      CALL SETRVS ('-23.32, 3.56E4, 2.295, 2.29', VWWNLM, ASIZ)
      CALL SETRVS ('0.298, 0.736, 0.22, 0.89', PJVPL2, ASIZ)
      CALL REVMM (VWWNLM, PJVPL2, ERRIND, RA33)

C  UMIN = UMAX
      CALL SETRVS ('3.56E4, 3.56E4, 2.29, 2.295', VWWNLM, ASIZ)
      CALL REVMM (VWWNLM, PJVPL2, ERRIND, RA33)
