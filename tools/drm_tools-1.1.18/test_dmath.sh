...
EOD
# cat /tmp/input1.txt | eval valgrind --leak-check=full  --show-reachable=yes $NEWPROG >$TMPDIR/new.txt
# cat /tmp/input1.txt | eval valgrind $NEWPROG  >$TMPDIR/new.txt
 cat /tmp/input1.txt | eval $NEWPROG >$TMPDIR/new.txt
 let TESTED+=1
...
dotest; echo "Logs, exponents, hyperbolic functions, bitwise"
